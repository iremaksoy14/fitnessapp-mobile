      ...
      db.transaction((tx) => {
        exercises.forEach((exercise) => {
          tx.executeSql(
            ...
            (id, title, category, video, durationOptions, image) 
            VALUES (?, ?, ?, ?, ?, ?)`,
            [
              exercise.id,
              exercise.title,
              exercise.category,
              exercise.video,
              JSON.stringify(exercise.durationOptions),
              JSON.stringify(exercise.image), // veya image yolunu string olarak ver
            ]
          );
          ...
          exercise.steps.forEach((step) => {
            tx.executeSql(
              ...
              (id, exerciseId, name, gif) 
              VALUES (?, ?, ?, ?)`,
              [step.id, exercise.id, step.name, step.gif]
            );
          });
        });
      });
